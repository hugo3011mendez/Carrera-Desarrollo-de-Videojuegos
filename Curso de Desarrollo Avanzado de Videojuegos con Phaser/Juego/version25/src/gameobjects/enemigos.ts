...
export default class Enemigos extends Phaser.Physics.Arcade.Group { // Hereda de Arcade Group : Un grupo de spritesheets con físicas
    private escena : Nivel1; // La escena en la que se van a aplicar
    ...
    constructor(escena: Nivel1, nombreObjeto:string, idObjeto:string, animacionObjeto:string, velocidad:number, rect: any){
        ...
        this.children.entries.map((enemigo:any)=>{
            ...
            enemigo.body.setSize(rect.size.x, rect.size.x,); // Tamaño
            enemigo.body.setOffset(rect.offset.x, rect.offset.y); // Offset
