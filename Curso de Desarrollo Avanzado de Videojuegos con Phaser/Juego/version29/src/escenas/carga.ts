...
export default class Carga extends Phaser.Scene 
{
  ...
  */
  ...
  preload (): void {
    ...
    this.load.spritesheet(Constante.ENEMIGOS.CHICKEN.ID, 'imagenes/enemigos/chicken.png', { frameWidth: 32, frameHeight: 34 });
    this.load.spritesheet(Constante.ENEMIGOS.MUSHROOM.ID, 'imagenes/enemigos/mushroom.png', { frameWidth: 32, frameHeight: 32 });
    this.load.spritesheet(Constante.ENEMIGOS.RADISH.ID, 'imagenes/enemigos/radish.png', { frameWidth: 30, frameHeight: 38 });
  
    //Explosión
    this.load.spritesheet(Constante.ENEMIGOS.EXPLOSION.ID, 'imagenes/enemigos/explosion.png', {frameWidth: 38, frameHeight: 38});

    //Plataformas móviles
    this.load.image(Constante.PLATAFORMAMOVIL.ID, 'imagenes/objetos/plataformamovil.png');

    //Sonidos
    // Cargo los sonidos correspondientes
    this.load.audio(Constante.SONIDOS.EFECTOS.SALTAR, 'sonidos/efectos/saltar.ogg');
    this.load.audio(Constante.SONIDOS.EFECTOS.CAERSOBREENEMIGO, 'sonidos/efectos/caersobre.ogg');
    this.load.audio(Constante.SONIDOS.EFECTOS.QUITARVIDA, 'sonidos/efectos/vida.ogg');
    this.load.audio(Constante.SONIDOS.EFECTOS.RECOLECTAR, 'sonidos/efectos/recolectar.ogg');              
    
    // Hay 3 identificadores en función del nivel, por lo que los uso en un bucle
    for (let i=0; i<=2; i++) this.load.audio(Constante.SONIDOS.BANDASONORA + i, `sonidos/bandasonora/cartoongame${i}.ogg`);
    
    // Recolectables
    // Cargo su Spritesheet indicando sus tamaños
    this.load.spritesheet(Constante.RECOLECTABLES.PLATANO.ID, 'imagenes/objetos/platano.png', {frameWidth:32, frameHeight:32});
    this.load.spritesheet(Constante.RECOLECTABLES.CEREZA.ID, 'imagenes/objetos/cereza.png', {frameWidth:32, frameHeight:32});
    this.load.spritesheet(Constante.RECOLECTABLES.PINA.ID, 'imagenes/objetos/pina.png', {frameWidth:32, frameHeight:32});
  }   
}